	...
	if not BWQ.hasUnlockedWorldQuests then
		...
		elseif (BWQ.expansion == CONSTANTS.EXPANSIONS.DRAGONFLIGHT) then
			...
			BWQ.hasUnlockedWorldQuests = (BWQ.expansion == CONSTANTS.EXPANSIONS.SHADOWLANDS and UnitLevel("player") >= 51 and C_QuestLog.IsQuestFlaggedCompleted(57559))
				...
				or (BWQ.expansion == CONSTANTS.EXPANSIONS.LEGION and (UnitLevel("player") >= 45 or (PlayerIsTimerunning() and PlayerGetTimerunningSeasonID() == 2 and UnitLevel("player") >= 30) and C_QuestLog.IsQuestFlaggedCompleted(41694)) or C_QuestLog.IsOnQuest(45812) or C_QuestLog.IsOnQuest(45838) or C_QuestLog.IsOnQuest(45840) or C_QuestLog.IsOnQuest(45839)) -- broken isles
		end
	end
